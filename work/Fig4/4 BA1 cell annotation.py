...
#df_markers = sc.get.rank_genes_groups_df(mydata,
 #                                        key='rank_genes_groups', #rank_genes_groups
 #                                        group=mydata.obs['major_celltype'].unique())

#df_markers=df_markers.dropna()
print(rank_results)
...
all_groups_results.to_csv('BA1-prediction-differential_expression_all_groups.csv', index=False)
...
# 绘制并保存聚类结果
plot_clusters_and_save_image(title="Region of interest 1", gdf=gdf, img=img, adata=mydata,  color_by_obs='clusters1', output_name=dir_base+"BA1-image_clustering.ROI1.tiff")


mydata.write('Fig4跑图BA1.h5ad', compression="gzip")