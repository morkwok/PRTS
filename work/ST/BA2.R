...
setwd("D:/R/002/ST/data/BA2/place")
...
library(Seurat)
library(hdf5r)
mydata = Load10X_Spatial(data.dir = "D:/R/002/ST/data/BA2/CytAssist_FFPE_Mouse_Brain_Rep2_spatial",
                         filename = "CytAssist_FFPE_Mouse_Brain_Rep2_filtered_feature_bc_matrix.h5",
                         assay = "Spatial", 
                         slice = "Mouse Brain Coronal Section 2 (FFPE)",
                         filter.matrix = T)
...
pdf(file="S9B1.pdf",width = 8,height = 8)
plot_grid(plot2)
dev.off()
...
pdf(file="S9B2.pdf",width = 8,height = 8)
plot_grid(plot2)
dev.off()
...
pdf(file="S9D.pdf",width = 15,height = 15)
SpatialFeaturePlot(mydata,features = c("Kcnma1","Plp1","Ptgds","Ttr","Apoe"))
dev.off()
...
pdf(file="注释BA2.pdf",width = 25,height = 10)
DotPlot(mydata, features = unique(top5_markers$gene)) + 
  RotatedAxis()
dev.off()
