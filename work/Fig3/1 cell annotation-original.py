...
sc.tl.leiden(count_area_filtered_adata, resolution=1.8, key_added="clusters1")
...
#1200移除小的聚类，例如移除大小为5以下的聚类
count_area_filtered_adata = count_area_filtered_adata[count_area_filtered_adata.obs['clusters1'].map(count_area_filtered_adata.obs['clusters1'].value_counts() >= 1200), :]

dir_base = 'E:/SCI/002/picture/Fig3/'

#plot_clusters_and_save_image(title="Complete histological image", gdf=gdf, img=img, adata=count_area_filtered_adata,  color_by_obs='clusters1', output_name=dir_base+"原图对照未注释.tiff")
...
#注释参考
sc.tl.rank_genes_groups(count_area_filtered_adata, groupby='clusters1', method='wilcoxon')
sc.pl.rank_genes_groups(count_area_filtered_adata, n_genes=25, sharey=False)

rank_results = count_area_filtered_adata.uns['rank_genes_groups']
...
group_labels = count_area_filtered_adata.obs[rank_results['params']['groupby']].unique()
for group_label in group_labels:
    group_df = sc.get.rank_genes_groups_df(count_area_filtered_adata, group=group_label)
...
all_groups_results.to_csv(dir_base+'Frozen-real-differential_expression_all_groups.csv', index=False)

count_area_filtered_adata.write('Fig3 run（real）.h5ad', compression="gzip")